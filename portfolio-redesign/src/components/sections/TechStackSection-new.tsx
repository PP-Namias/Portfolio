  ...
  return (
    ...
    >
      ...
      <div className="space-y-4">
        {Array.isArray(techStackData) && techStackData.map((category: any, categoryIndex: number) => {
        ...
        <div className="grid grid-cols-2 gap-4 text-center">
          <div>
            <div className="text-lg font-bold text-[var(--color-accent)]">
              {Array.isArray(techStackData) ? 
                techStackData.reduce((total: number, cat: any) => total + (cat.technologies?.length || 0), 0) : 0}
            </div>
            <div className="text-xs text-[var(--color-text-muted)]">Technologies</div>
          </div>
          <div>
            <div className="text-lg font-bold text-[var(--color-accent)]">
              {Array.isArray(techStackData) ? techStackData.length : 0}
            </div>
