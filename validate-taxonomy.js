...
const APPROVED_PROJECT_CATEGORIES = [
	"Web Development",
	"AI & Automation",
...
class ContentValidator {
	...
	validateCategory(category, isProject = false) {
		const approvedCategories = isProject
			? APPROVED_PROJECT_CATEGORIES
			: APPROVED_BLOG_CATEGORIES;
	...
	validateContent(filePath, isProject = false) {
		...
		try {
			...
			if (frontmatter.tags && frontmatter.tags.length > 0) {
				...
				const hasTechnologyTag = frontmatter.tags.some((tag) =>
					technologyTags.includes(tag),
				);
				const hasTopicTag = frontmatter.tags.some((tag) =>
			...
			if (frontmatter.category || frontmatter.tags) {
				...
				console.log(
					`   🏷️  Tags: ${frontmatter.tags ? frontmatter.tags.join(", ") : "None"}`,
				);
			}
		} catch (error) {
	...
	generateReport() {
		...
		console.log(
			`📈 Success Rate: ${successRate}% (${this.validFiles}/${this.totalFiles} files)`,
		);
		...
		if (this.errors.length === 0 && this.warnings.length === 0) {
			...
			console.log(
				"4. Use the migration script to automate fixes where possible",
			);
		}
		...
		console.log(
			...
			APPROVED_BLOG_CATEGORIES.join(", "),
		);
		console.log(
			"- Approved project categories:",
			APPROVED_PROJECT_CATEGORIES.join(", "),
		);
