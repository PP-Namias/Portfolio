...
function standardizeTags(tags) {
	...
	standardizedTags = standardizedTags.filter((tag) =>
		APPROVED_TAGS.includes(tag),
	);
...
function processFile(filePath, categoryMap) {
	try {
		...
		console.log(
			`   Current tags: ${frontmatter.tags ? frontmatter.tags.join(", ") : "none"}`,
		);

		// Map category
		const newCategory =
			categoryMap[frontmatter.category] || frontmatter.category;
