        ...
        {visibleProjects.map((project, index) => {
          ...
          return (
            <motion.article
              ...
              onMouseLeave={() => setActiveProjectTitle((current) => (current === project.title ? null : current))}
              className={`group relative overflow-hidden rounded-xl border bg-white dark:bg-card-bg-dark ${
                target
                  ? 'cursor-pointer border-border-light dark:border-border-dark hover:border-accent-pink/50'
                  : 'cursor-default border-border-light dark:border-border-dark'
              }`}
            >
              {target && (
                <a
                  ...
                  onBlur={() => setActiveProjectTitle((current) => (current === project.title ? null : current))}
                >
                  <span className="sr-only">Open detailed view for {project.title}</span>
                </a>
              )}
              ...
              <div className="relative h-44 overflow-hidden border-b border-border-light dark:border-border-dark sm:h-48">
                ...
                {target && (
                  <div className="absolute right-3 top-3 flex items-center gap-1 rounded-full border border-accent-pink/40 bg-black/45 px-2 py-0.5 text-[11px] font-medium text-accent-pink backdrop-blur-sm">
              ...
              <div className="relative p-4">
                  ...
                  {!reduceMotion && (
                    <>
                      <motion.span
                        ...
                        animate={
                          isActive
                            ? { x: [0, -1, 1, 0], opacity: [0, 0.6, 0.2, 0.55] }
                            : { x: 0, opacity: 0 }
                        }
                      ...
                      <motion.span
                        ...
                        animate={
                          isActive
                            ? { x: [0, 1, -1, 0], opacity: [0, 0.55, 0.15, 0.45] }
                            : { x: 0, opacity: 0 }
                        }
                ...
                >
                  <p className="text-[11px] text-text-secondary-light dark:text-text-secondary-dark">
                    {target
                      ? 'Hover or focus reveals cyber-style metadata. Click the card to inspect the full project experience.'
                      : 'Hover or focus reveals project metadata. No destination link is configured yet.'}
                  </p>
