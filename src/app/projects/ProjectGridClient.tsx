...
export function ProjectGridClient({ projects }: Readonly<ProjectGridClientProps>) {
  ...
  return (
    <div>
          ...
          >
            ...
            <span className="relative z-10">
              ...
              <span className="ml-1.5 text-[11px] text-text-muted-light dark:text-text-muted-dark">
                ({tab.id === 'all' ? allProjects.length : tab.id === 'live' ? liveProjects.length : showcaseProjects.length})
              </span>
            </span>
          </button>
        ))}
      ...
      {/* Project grid */}
      <div id={`tabpanel-listing-${activeTab}`} role="tabpanel" aria-labelledby={`tab-listing-${activeTab}`}>
        <div
          key={activeTab}
...
function ListingCard({ project, index, reduceMotion }: Readonly<ListingCardProps>) {
  ...
  const cardTransition = reduceMotion
    ? { duration: 0 }
    : { duration: 0.3, ease: [0.25, 0.1, 0.25, 1] as [number, number, number, number], delay: index * 0.04 };
