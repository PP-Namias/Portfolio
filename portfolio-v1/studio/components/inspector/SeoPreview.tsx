...
export function SeoPreview() {
  ...
  if (!seo.title) {
    ...
    issues.push({
      label: `Title is short (${titleLen} chars; aim for ${WARN_TITLE_MIN}-${META_TITLE_LIMIT})`,
      tone: 'warn',
    })
  } else if (titleLen > META_TITLE_LIMIT) {
    issues.push({
      label: `Title exceeds ${META_TITLE_LIMIT} chars and will be truncated in search results.`,
      tone: 'warn',
    })
  }
  ...
  if (!seo.description) {
    ...
    issues.push({
      label: `Description is short (${descLen} chars; aim for ${WARN_DESC_MIN}-${META_DESC_LIMIT})`,
      tone: 'warn',
    })
  } else if (descLen > META_DESC_LIMIT) {
    issues.push({
      label: `Description exceeds ${META_DESC_LIMIT} chars and will be truncated.`,
      tone: 'warn',
    })
  }
          ...
          {imageUrl ? (
            ...
            <div>
              ...
              <div style={{fontSize: 14, opacity: 0.7, marginTop: 8}}>
                {trim(seo.description, 140) || 'No description'}
              </div>
            </div>
          )}
        </div>
            ...
            {issues.map((issue) => (
              <li
                ...
                style={{
                  ...
                  borderRadius: 8,
                  background:
                    issue.tone === 'error' ? 'rgba(239,68,68,0.12)' : 'rgba(245,158,11,0.12)',
