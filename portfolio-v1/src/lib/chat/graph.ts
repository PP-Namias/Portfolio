...
import { generateWithGemini, generateWithOpenAI, isMultiProviderEnabled, classifyProviderError, streamWithGemini } from '@/app/api/chat/lib/providers';
...
function createGenerateNode(options: ChatGraphOptions) {
  return async function generateNode(state: GraphStateType): Promise<Partial<GraphStateType>> {
    ...
    try {
      if (options.onToken) {
        const streamResult = await streamWithGemini(
          messageForProvider,
          state.history,
          systemPrompt,
...
export async function runChatGraph(params: {
  ...
  const graph = onToken || onToolCall
    ? createChatGraph({ onToken, onToolCall })
    : getCompiledGraph();
