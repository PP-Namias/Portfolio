...
function buildSignaturePayload({
  ...
  quality,
  expiresAt,
}: MediaGatewaySignatureInput): string {
  return [targetUrl, width ?? '', quality ?? '', expiresAt ?? ''].join('|')
}

export function createMediaGatewaySignature(
  input: MediaGatewaySignatureInput
...
export function verifyMediaGatewaySignature(
  input: MediaGatewaySignatureInput & { signature?: string | null }
): {
  valid: boolean
  expired: boolean
} {
  ...
  const expected = createHmac('sha256', secret).update(buildSignaturePayload(input)).digest()
...
export function buildMediaGatewayUrl(
  ...
  options: { width?: number; quality?: number; sign?: boolean; label?: string } = {}
): string {
  ...
  const segment = buildCleanGatewaySegment(encodedTarget, normalized, options.label)
  ...
  const signature = options.sign
    ? createMediaGatewaySignature({ targetUrl: normalized, width, quality })
    : null
  ...
  return `${MEDIA_ROUTE_PREFIX}/${SANITY_NAMESPACE}/${segment}${querySuffix}`
}
