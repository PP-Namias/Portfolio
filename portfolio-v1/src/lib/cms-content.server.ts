...
export async function querySanity<T>(
  query: string,
  options?: { tags?: string[] }
): Promise<T | null> {
  ...
  const result = await getOrFetch<T | null>(
    cacheKey,
    () => querySanityFresh<T>(query, projectId, perspective),
    {
      ttlMs: CACHE_TTL_MS,
...
async function querySanityFresh<T>(
  query: string,
  projectId: string,
  perspective: string
): Promise<T | null> {
...
const getCmsContentImpl = async (): Promise<CmsContent> => {
  ...
  const socialLinks = (() => {
    const mapped = (profileDoc?.socialLinks ?? [])
      .map(mapSocialLink)
  ...
  const certifications: Certification[] = (certificationDocs ?? []).map((certification, index) => ({
    ...
    image:
      buildMediaGatewayUrl(certification.imageUrl || '', {
        width: 320,
        quality: 85,
        sign: true,
  ...
  return {
    seoSettings: {
      ...
      siteTitle:
        siteSettingsDoc?.ogTitle || siteSettingsDoc?.defaultMetaTitle || fb.seoSettings.siteTitle,
      siteDescription:
        siteSettingsDoc?.ogDescription ||
        siteSettingsDoc?.defaultMetaDescription ||
      ...
      twitterImageUrl:
        siteSettingsDoc?.twitterImageUrl ||
        siteSettingsDoc?.ogImageUrl ||
        fb.seoSettings.twitterImageUrl,
    ...
    siteSettings: {
      ...
      blog: {
        ...
        description:
          siteSettingsDoc?.blog?.description ||
...
export async function getProjectBySlug(slug: string): Promise<Project | null> {
  ...
  try {
    ...
    return {
      title: doc.title || '',
      image:
        buildMediaGatewayUrl(doc.imageUrl || '', {
          width: 960,
          quality: 85,
          sign: true,
      ...
      role: doc.role,
      impactMetrics: (doc.achievements || []).map((a: string, i: number) => ({
        label: `Highlight ${i + 1}`,
        value: a,
      })),
      featuredRank: doc.featuredRank || null,
      status: (doc.status as Project['status']) || undefined,
      gallery: (doc.galleryItems ?? []).map(
        (g: {
          url?: string
          alt?: string
          caption?: string
