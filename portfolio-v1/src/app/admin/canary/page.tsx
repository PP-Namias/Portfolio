...
export default function CanaryDashboard() {
  ...
  const {
    ...
    isLoading,
    mutate,
  } = useSWR<CanaryStats>('/api/canary/stats', fetcher, { revalidateOnFocus: false })
      ...
      <div className="min-h-screen bg-gray-900 text-white p-8">
        <div className="max-w-6xl mx-auto">
          <h1 className="text-3xl font-bold mb-8 text-red-500">
            Error: Failed to fetch canary stats
          </h1>
        </div>
      </div>
    )
                      ...
                      >
                        ...
                        <div className="text-right">
                          ...
                          <div className="text-xs text-gray-500">
                            {token.lastTriggered
                              ? new Date(token.lastTriggered).toLocaleString('en-US', {
                                  timeZone: 'UTC',
                                })
                              : 'Never'}
                          </div>
                  ...
                  <div className="space-y-2">
                    {stats.recentTriggers.length === 0 ? (
                      ...
                      stats.recentTriggers.map((trigger) => (
                        <div key={trigger.id} className="bg-gray-800 p-4 rounded-lg">
                          <div className="flex justify-between">
                            <div className="font-semibold">{trigger.tokenName}</div>
                            <div className="text-sm text-gray-400">
                              {new Date(trigger.timestamp).toLocaleString('en-US', {
                                timeZone: 'UTC',
                              })}
                            </div>
                          </div>
