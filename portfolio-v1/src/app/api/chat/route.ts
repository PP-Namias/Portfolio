...
import { generateWithProviderChain, getProviderHealth } from './lib/providers'
import { buildSystemPromptParts } from './lib/promptBuilder'
...
function isValidationError(result: ParsedChatRequest | ValidationError): result is ValidationError {
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // The prompt is split so Claude can mark the CMS half as a cache prefix;
    // the other providers receive the two halves concatenated.
    const promptParts = buildSystemPromptParts(chatDataContext, ragContext)

    const { result: providerResult, attempts: providerAttempts } = await generateWithProviderChain(
      message,
      history,
      promptParts
    )
    ...
    if (providerResult) {
      logEvent('info', 'chat_provider_success', {
        ...
        totalLatencyMs: Date.now() - requestStartedAt,
        failoverCount: providerAttempts.length - 1,
        providerAttempts,
      })

      return withRequestId(
        NextResponse.json({ message: providerResult.message, fallback: false }),
        requestId
      )
    }
    ...
    logEvent('warn', 'chat_fallback_response', {
      requestId,
      totalLatencyMs: Date.now() - requestStartedAt,
      failoverCount: providerAttempts.length,
      providerAttempts,
    })
...
async function handleStreamingResponse(
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      try {
        controller.enqueue(
          encoder.encode(encodeSSE('status', { step: 'start', threadId: resolvedThreadId }))
        )
        ...
        const result = await runChatGraph({
          ...
          onStatus: (step) => {
            controller.enqueue(
              encoder.encode(encodeSSE('status', { step, threadId: resolvedThreadId }))
            )
          },
        })
