import { ChatDataContext, ExperienceData, ProjectData, TechnologyData, SocialData } from './types'
...
const catalog: CatalogEntry[] = [
  ...
  },
  {
    name: 'who_is',
    match: (msg) =>
      msg.includes('who is keneth') ||
      msg.includes('who are you') ||
      msg.includes('tell me about yourself') ||
      msg.includes('tell me about keneth') ||
      msg.includes('about keneth') ||
    ...
    response: (data) => {
      ...
      const summary = data.profile.summary || ''
      const techs =
        data.profile.highlights?.primaryTechnologies?.slice(0, 5).join(', ') ||
  ...
  },
  {
    ...
    response: (data, msg) => {
      const matched = data.projects.find(
        (p) => p.title && msg.includes(p.title.toLowerCase().slice(0, 10))
      )
      if (matched) return formatProject(matched)
      const fuzzy = data.projects.find((p) => {
        const t = (p.title || '').toLowerCase()
        return (
          knownProjectSlugs.some((s) => msg.includes(s)) ||
  ...
  },
  {
    ...
    response: (data, msg) => {
      ...
      if (matched) {
        ...
        const techs = matched.technologies?.length
          ? ` using ${matched.technologies.join(', ')}`
          : ''
  ...
  },
  {
    name: 'skill_detail',
    match: (msg) =>
      knownTechNames.some((name) => msg.includes(name)) && !msg.includes('experience'),
    response: (data, msg) => {
      const matched = data.technologies.find((t) => t.name && msg.includes(t.name.toLowerCase()))
      if (matched)
        return `${matched.name} — proficiency: ${matched.proficiency || 0}%. Category: ${matched.category || 'General'}.`
  ...
  },
  {
    ...
    match: (msg) =>
      ...
      (msg.includes('know') &&
        (msg.includes('tech') || msg.includes('code') || msg.includes('program'))),
    response: (data) => {
      ...
      const ai = [
        ...topByCategory(data.technologies, 'AI', 4),
        ...topByCategory(data.technologies, 'Data Science', 3),
      ]
        .filter(Boolean)
        .join(', ')
  ...
  },
  {
    ...
    match: (msg) =>
      ...
      msg.includes('hackerrank') ||
      msg.includes('tesda') ||
      msg.includes('license') ||
      msg.includes('accreditation'),
    response: (data) => {
  ...
  },
  {
    ...
    response: (data) => {
      ...
      const github =
        data.profile.github || findSocial(data.socials, 'github') || 'https://github.com/PP-Namias'
      const linkedin =
        data.profile.linkedin ||
        findSocial(data.socials, 'linkedin') ||
  ...
  },
  {
    ...
    match: (msg) =>
      ...
      msg.includes('call') ||
      msg.includes('collaborat') ||
      msg.includes('consult') ||
      msg.includes('discuss') ||
      (msg.includes('talk') && msg.includes('project')) ||
  ...
  },
  {
    ...
    match: (msg) =>
      ...
      msg.includes('accomplishment') ||
      msg.includes('milestone') ||
      msg.includes('highlight') ||
      msg.includes('standout') ||
      msg.includes('award') ||
  ...
  },
  {
    ...
    match: (msg) =>
      ...
      msg.includes('member of') ||
      msg.includes('society') ||
      msg.includes('psia') ||
      msg.includes('aiap') ||
      msg.includes('philippine'),
    response: (data) => {
  ...
  },
  {
    ...
    match: (msg) =>
      ...
      msg.includes('live') ||
      (msg.includes('where') &&
        (msg.includes('keneth') || msg.includes('you') || msg.includes('he'))) ||
      msg.includes('manila') ||
      msg.includes('philippines'),
    response: (data) => {
