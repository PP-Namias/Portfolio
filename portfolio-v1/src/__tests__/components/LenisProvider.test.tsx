...
vi.mock('lenis/react', () => ({
  ReactLenis: ({ children, root, options, className }: any) => (
    <div data-testid="react-lenis" data-root={root} data-options={JSON.stringify(options)} className={className}>{children}</div>
  ),
}));

vi.mock('next/dynamic', () => ({
  default: () => {
    const LenisInner = ({ children }: { children: React.ReactNode }) => {
      const { ReactLenis } = require('lenis/react');
      return <ReactLenis root options={{ lerp: 0.12, duration: 1.2, smoothWheel: true, touchMultiplier: 1.5, wheelMultiplier: 1 }}>{children}</ReactLenis>;
    };
    return LenisInner;
  },
}));
...
describe('LenisInner', () => {
  it('wraps children in ReactLenis with root and correct options', () => {
    render(<LenisInner><div data-testid="child">Content</div></LenisInner>);
  ...
  it('passes root=true to ReactLenis', () => {
    render(<LenisInner><span>test</span></LenisInner>);
    expect(screen.getByTestId('react-lenis')).toHaveAttribute('data-root', 'true');
  });

  it('passes smooth wheel options', () => {
    render(<LenisInner><span>test</span></LenisInner>);
    const options = JSON.parse(screen.getByTestId('react-lenis').getAttribute('data-options') || '{}');
