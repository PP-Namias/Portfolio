...
describe('robots.txt route', () => {
  it('returns a catch-all allow rule with utility routes disallowed', () => {
    const res = robots()
  ...
  it('explicitly allows major web and image crawlers', () => {
    const res = robots()
    const web = res.rules.find(
      (r) => Array.isArray(r.userAgent) && r.userAgent.includes('Googlebot')
    )
    ...
    expect(web?.disallow).toEqual(['/admin', '/offline', '/studio'])
  })
  ...
  it('explicitly allows AI model and answer engine crawlers', () => {
    ...
    expect(ai?.disallow).toEqual(['/admin', '/offline', '/studio'])
  })
